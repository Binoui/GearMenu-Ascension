  ...
  if not info.notCheckable then
    ...
    if info.customCheckIconAtlas or info.customCheckIconTexture then
      ...
      if info.customCheckIconAtlas then
        -- SetAtlas doesn't exist in WoW 3.3.5, use SetTexture with the atlas as texture path
        if check.SetAtlas then
          check:SetAtlas(info.customCheckIconAtlas)
        else
          check:SetTexture(info.customCheckIconAtlas)
        end
        if uncheck.SetAtlas then
          uncheck:SetAtlas(info.customUncheckIconAtlas or info.customCheckIconAtlas)
        else
          uncheck:SetTexture(info.customUncheckIconAtlas or info.customCheckIconAtlas)
        end
      else
